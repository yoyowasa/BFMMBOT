...
try:
    from pydantic import BaseModel, Field, ConfigDict  # 型検査モデル（v2）
except ImportError:  # v1 互換
    from pydantic import BaseModel, Field  # 型検査モデル（v1）
    ConfigDict = None
...
class GateCfg(BaseModel):
    ...
    max_inflight_per_key: int | None = None
...
class RiskCfg(BaseModel):
    # YAML 側の risk.inv_capping / risk.auto_reduce など未スキーマ項目も model_extra に保持したい
    model_config = ConfigDict(extra="allow") if ConfigDict is not None else None
    if ConfigDict is None:
        class Config:
            extra = "allow"
    max_inventory: float
    limit_qty: float | int | None = None
    kill: KillCfg | None = None

class GuardCfg(BaseModel):
    # guard.feed_health / guard.caution など未スキーマ項目も model_extra に保持したい
    model_config = ConfigDict(extra="allow") if ConfigDict is not None else None
    if ConfigDict is None:
        class Config:
            extra = "allow"
...
class Config(BaseModel):
    ...
    canary_minutes: float | int | None = None
    dry_run_max_sec: float | int | None = None
